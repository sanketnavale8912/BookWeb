...
public class RegistrationDaoImpl implements RegistrationDao {
	...
	public int addRegistrationDao(Registration reg) throws DaoException
	{
		...
		if(cnt>0){
			...
			if(reg.getRole().equalsIgnoreCase("ROLE_USER")){
			  status = jdbcTemplate.update(sql, new Object[]{ WordUtils.capitalizeFully(reg.getFirstName()), WordUtils.capitalizeFully(reg.getLastname()),reg.getEmail(),reg.getPassword(),reg.getEmail_verification_token(),reg.getUser_type(),reg.getStatus(),reg.getRole(),reg.getDate(),reg.getIsEmailAlert() });
			}else{
				status = jdbcTemplate.update(sql, new Object[]{ reg.getFirstName(), reg.getLastname(),reg.getEmail(),reg.getPhone().replace("\"", ""),reg.getCountry(),reg.getState(),reg.getCompanyName(),reg.getClientSecret(),reg.getPassword(),reg.getEmail_verification_token(),reg.getUser_type(),reg.getStatus(),reg.getRole(),reg.getDate(),reg.getProject(),reg.getIsEmailAlert()});	
			}
		}
	    ...
	    Registration listContact = jdbcTemplate.query(sql, new Object[] { email }, new ResultSetExtractor<Registration>()
	    {
			...
			{				
				if(rs.next()){
		    		...
		    		r.setIsEmailAlert(rs.getInt("emailalert"));
		    		return r;
		    		}
				return null;				
			}
	...
	public String checkValidResetPassword(ResetPassword resetPassword) {
		...
		List<Integer> ids = jdbcTemplate.queryForList(checkValidResetPassword,Integer.class, resetPassword.getUserid(),resetPassword.getToken(),resetPassword.getEmail());
		if(ids.size()>0){
				flag = "1";
		}
		return flag;
	...
	public int updateRegistration(Registration registration) throws DaoException {
		...
		String sql = queryProps.getProperty("updateRegistration");
		
		int status = jdbcTemplate.update(sql,WordUtils.capitalizeFully(registration.getFirstName()),WordUtils.capitalizeFully(registration.getLastname()),registration.getMobile(),registration.getPhone(),registration.getCountry(),registration.getState(),registration.getCity(),registration.getAddress(),registration.getPincode(),registration.getId(),registration.getEmail());
	...
	public int updatePassword(Registration registration) {
		...
		String sql = queryProps.getProperty("updatePassword");
		int status = jdbcTemplate.update(sql, registration.getPassword(),registration.getEmail(),registration.getId());
		return status;
	}
